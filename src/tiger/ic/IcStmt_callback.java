...
import java.util.*;
public class IcStmt_callback extends IcStmt{
	public IcFun called;
	public ArrayList<IcOper> args = new ArrayList<IcOper>();
	public IcVar result;
	public IcStmt_callback(IcFun f,int n,IcFun fc,IcVar v){
		super(f,n);
		called = fc;
		result = v;
	}
	...
	public String toString(){
		...
		tmp.append(num+":"+"call function "+called.name+" to "+result+" with:");
	...
	public void gen_mips(){
		...
		if(compiler_cg_verbose)
			s.append("#"+"call function "+called.name+"with "+args.size()+" args (result to "+result+" )\n");
		...
		String x = Cg_solve_oper(result,s,1,fun);
		s.append(Mips_move(x,"$v0"));
		//option wb
		Cg_write_var(result,x,s,fun);
